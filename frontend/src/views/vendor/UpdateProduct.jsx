...
function UpdateProduct() {
	...
	const originalGalleryImages = useRef([]);
	const originalProductImage = useRef("");

	// fetch product
	const fetchProduct = async () => {
		try {
			const response = await axios.api_instance.get(
				`vendor/vendor-update-product/${userData?.vendor_id}/${params.pid}`
			);
			...
			originalGalleryImages.current = response.data?.gallery?.map(
				(item, index) => {
					return item.image;
				}
			);
			originalProductImage.current = response.data.image;
		} catch (e) {
			console.log(e);
		}
	};
	...
	const fetchImageAndConvert = async (imageURL) => {
		try {
	...
	const handleSubmit = async (event) => {
		...
		Object.entries(product).forEach(([key, value]) => {
			if (key === "image" && value) {
				if (!originalProductImage.current.includes(value)) {
		...
		for (const [index, item] of gallery.entries()) {
			if (item.image) {
				if (originalGalleryImages.current.includes(item.image)) {
					try {
						const imageBlob = await fetchImageAndConvert(
							item.image
						);
						formdata.append(`gallery[${index}][image]`, imageBlob);
					} catch (e) {
						console.log(`Gallery appendation error: ${e}`);
					}
				} else {
					formdata.append(
						`gallery[${index}][image]`,
		...
		try {
			...
			Swal.fire({
				title: "Product update failed due to unknown error",
				icon: "error",
			});
		}
	};

	console.log(originalGalleryImages.current);
